  ...
  return Mn.CompositeView.extend({
    ...
    __createChildView: function() {
      ...
      for (var i = 0; i < this.columns.length; i++) {
        ...
        attr += this.columns[i].cls
          ? ' class="col ' + this.columns[i].cls + '"'
          : ' class="col"';

        headerAttr = this.columns[i].width
          ? ' style="width:' + width + ';"'
          : '';
        headerAttr += this.columns[i].headerCls
          ? ' class="col h-col ' + this.columns[i].headerCls + '"'
          : ' class="col h-col"';
        ...
        if (this.columns[i].renderer) {
          tpl +=
            ...
            attr +
            '>' +
            this.columns[i].renderer.call(this) +
            '</div>';
        } else {
          tpl +=
            '<div' +
            attr +
            '>' +
            renderer(this.columns[i].dataIndex) +
            '</div>';
        }
        tplHeader +=
          '<div' + headerAttr + '>' + this.columns[i].header + '</div>';
      }
          ...
          '\
			        <div class="panel panel-default">\
			            ...
			            <div class="panel-body">\
			            	<div class="grid-body">\
			            		<div class="thead-outer"><div class="thead">' +
          tplHeader +
          '</div></div>\
			            		<div class="tbody"></div>\
			            	</div>\
